...
def deprecated_get_fda_eua_parsed_data(merged):
    ...
    if merged:
        with open(JSON_FILE_PATH_FOR_FDA_EUA_merged_PARSED_DATA, "r", encoding="utf8") as f:
            fda_eua_merged_parsed_data = json.load(f)

        return fda_eua_merged_parsed_data

    else:
        with open(JSON_FILE_PATH_FOR_FDA_EUA_iv_PARSED_DATA, "r", encoding="utf8") as f:
            fda_eua_iv_parsed_data = json.load(f)

        with open(JSON_FILE_PATH_FOR_FDA_EUA_high_complexity_PARSED_DATA, "r", encoding="utf8") as f:
